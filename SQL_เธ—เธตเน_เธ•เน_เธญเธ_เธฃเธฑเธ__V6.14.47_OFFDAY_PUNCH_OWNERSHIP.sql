...
-- 4) ไม่แก้ time_logs ดิบ และไม่เปลี่ยนกฎ Late/Absent/OT/Day-off quota
-- ============================================================================

begin;
set local statement_timeout = '0';
select pg_advisory_xact_lock(hashtext('timeclock_v61447_offday_punch_ownership'));
...
begin
  ...
  if to_regprocedure('public.get_expected_day_by_schedule(text,date,text,date,date)') is null then
    raise exception 'MISSING_FUNCTION: get_expected_day_by_schedule';
  end if;
...
begin
  ...
  base as (
    select
      ...
      public.get_expected_day_by_schedule(
        e.emp_code, d.work_date, e.pc, e.start_date, e.resign_date
  ...
  select
    ...
    jsonb_build_object(
      'rebuilt_at', now(),
      'match_version', 'SHIFT_WINDOW_V61447_OFFDAY_STRICT',
...
is 'V6.14.47: Shift-aware rebuild with strict IN/OUT ownership on non-working days; prevents previous overnight OUT from becoming next day-off IN.';
...
with suspicious as materialized (
  ...
  set
    raw_meta=coalesce(aw.raw_meta,'{}'::jsonb) || jsonb_build_object(
      'offday_punch_ownership_fix','V6.14.47',
