...
(() => {
  ...
  async function getMonthlySchedule(app, params) {
    ...
    let response = await withTimeout(
      ...
      30000,
      "โหลดปฏิทินกะตามรูปแบบการทำงาน V6.5.7"
    );
    if (response.error) {
    ...
    function renderSchedule() {
      ...
      for (const [emp, obj] of map) {
        ...
        for (let d=1; d<=days; d++) {
          ...
          const calcBits = [r.pattern_code, r.template_code, r.calculation_status, Number(r.overtime_minutes||0)>0?`OT ${(Number(r.overtime_minutes)/60).toFixed(1)} ชม.`:null, Number(r.waiting_minutes||0)>0?`รอ ${(Number(r.waiting_minutes)/60).toFixed(1)} ชม.`:null, r.comp_off_earned?"ได้วันหยุดชดเชย":null].filter(Boolean).join(" | ");
          const calcFlags = `${Number(r.overtime_minutes||0)>0?'<small class="schedule-calc-flag ot">OT</small>':''}${Number(r.waiting_minutes||0)>0?'<small class="schedule-calc-flag wait">W</small>':''}${r.comp_off_earned?'<small class="schedule-calc-flag comp">C</small>':''}`;
    ...
    async function saveShiftMaster() {
      ...
      try {
        const result = await window.TimeClockShiftAPI.upsertShiftMaster(window.TimeClockApp || { state }, {
          ...
          change_reason: "บันทึกจากหน้า HR Admin V6.5.7"
        });
...
/* ===== V6.5.7 CSV import + technician work patterns + calculation UI ===== */
(() => {
  'use strict';
  const $ = id => document.getElementById(id);
  ...
  function applyWorkPatternParameterVisibility(){
    ...
    const visible=wp.parameterAccessLoaded&&wp.canViewParameters===true;
    if(panel){
    ...
    if(addButton){
      addButton.classList.toggle('hidden',!wp.canManageParameters);
      addButton.disabled=!wp.canManageParameters;
    }
  }
  ...
  function bindV620(){
    ...
    document.addEventListener('click',e=>{const edit=e.target.closest('[data-edit-pattern]');if(edit&&wp.canManageParameters)openPattern(edit.dataset.editPattern);const assign=e.target.closest('[data-assign-pattern]');if(assign)openEmployeePattern(assign.dataset.assignPattern);});
...
(function TimeClockV650(){
  ...
  function openAttendance(key){const [emp,date]=key.split('|');app()?.switchPage?.('attendance');if($('attStart'))$('attStart').value=date;if($('attEnd'))$('attEnd').value=date;if($('attendanceGridSearch'))$('attendanceGridSearch').value=emp;setTimeout(()=>app()?.loadAttendance?.(),0);}
