(() => {
  ...
  async function savePayload(payload, reason, confirmNow=false, historyLabel="แก้ไขกะ"){
    ...
    try{
      const {error}=await app().state.client.rpc("ta_assign_shifts_bulk",{p_rows:payload,p_change_reason:reason,p_confirm_now:confirmNow});
  ...
  async function applyHistory(item,mode){const payload=(mode==="undo"?item.before:item.after).map(x=>({...x,note:`${mode} ${item.label}`}));app().showLoading(`กำลัง ${mode==="undo"?"ย้อนกลับ":"ทำซ้ำ"}...`);try{const {error}=await app().state.client.rpc("ta_assign_shifts_bulk",{p_rows:payload,p_change_reason:`${mode} ${item.label}`,p_confirm_now:false});if(error)throw error;(mode==="undo"?redoStack:undoStack).push(item);updateHistoryButtons();await app().loadSchedule();app().toast(mode==="undo"?"ย้อนกลับแล้ว":"ทำซ้ำแล้ว","success");}catch(err){app().toast(app().humanError(err),"error");}finally{app().hideLoading();}}
