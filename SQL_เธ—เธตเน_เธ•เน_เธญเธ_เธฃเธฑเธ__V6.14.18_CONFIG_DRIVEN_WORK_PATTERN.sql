...
--
-- หลักการ:
-- - ไม่มี Position Policy บังคับผู้จัดการแผนกเป็น TECH_5D
-- - รูปแบบ 5/6 วัน, กะตั้งต้น และวันหยุดอ้างอิงค่าที่กำหนดจาก Work Pattern
-- - วันเริ่มใช้ยังต้องผ่าน System Period Guard ของ V6.14.17
-- ============================================================================
...
-- ---------------------------------------------------------------------------
-- 2) Neutralize the V6.14.17 position helper for backward compatibility.
--    No position is allowed to force a Work Pattern anymore.
-- ---------------------------------------------------------------------------
...
begin
  ...
  return jsonb_build_object(
    ...
    'pc',v_pc,
    'policy_code','CONFIG_DRIVEN',
    'forced_pattern_code',null,
    'weekly_off_dows',null,
...
-- ---------------------------------------------------------------------------
-- 3) Canonical single save: selected Work Pattern is authoritative.
-- ---------------------------------------------------------------------------
...
begin
  ...
  -- V6.14.17 rule remains authoritative: closed Schedule/Certification period
  -- cannot be used as Work Pattern effective date, including HR Admin.
  ...
  -- Validate the selected DAY/NIGHT shift against the selected 5D/6D pattern.
  ...
  -- Individual weekly-off override stays disabled. The Work Pattern setting is
  -- the single source of truth for weekly days off.
  ...
  return coalesce(v_saved,'{}'::jsonb) || jsonb_build_object(
    ...
    'work_pattern_source','CONFIGURED_WORK_PATTERN',
...
-- ---------------------------------------------------------------------------
-- 4) Bulk save: every selected employee receives exactly the selected pattern.
-- ---------------------------------------------------------------------------
create or replace function public.ta_assign_employee_work_patterns_bulk_v61418(
  p_rows jsonb
)
returns jsonb
language plpgsql
...
begin
  ...
  loop
    ...
    v_saved := public.ta_assign_employee_work_pattern_v61418(
      v_emp,
      v_item->>'pattern_code',
  ...
  return jsonb_build_object(
    ...
    'position_policy_adjusted_rows',0,
...
-- ---------------------------------------------------------------------------
-- 5) Config-driven reader for Bulk UI.
-- ---------------------------------------------------------------------------
create or replace function public.ta_get_employee_pattern_default_shift_v61418(
  p_emp_codes text[],
  p_effective_date date default current_date
)
returns table(
...
revoke all on function public.ta_get_employee_pattern_default_shift_v61418(text[],date) from public;
grant execute on function public.ta_get_employee_pattern_default_shift_v61418(text[],date) to authenticated;

-- ---------------------------------------------------------------------------
-- 6) Compatibility RPCs: old V6.14.17 names no longer enforce position policy.
-- ---------------------------------------------------------------------------
...
create or replace function public.ta_assign_employee_work_patterns_bulk_v61417(p_rows jsonb)
returns jsonb
language sql
security definer
set search_path=public
as $$
  select public.ta_assign_employee_work_patterns_bulk_v61418(p_rows)
         || jsonb_build_object('compatibility_rpc','V6.14.17');
$$;
...
as $$
  select
    r.emp_code,r.assigned_pattern_code,r.effective_pattern_code,r.effective_weekly_off_dows,
    r.assigned_default_shift_code,r.effective_default_shift_code,r.shift_period,r.shift_name,
    r.shift_start_time,r.shift_end_time,r.default_shift_source,
    'CONFIG_DRIVEN'::text,null::text,true
  from public.ta_get_employee_pattern_default_shift_v61418(p_emp_codes,p_effective_date) r;
$$;
...
-- ---------------------------------------------------------------------------
-- 7) Shared resolver fallback restored to non-position logic.
--    Explicit employee assignment remains authoritative.
-- ---------------------------------------------------------------------------
...
-- ---------------------------------------------------------------------------
-- 8) Main lightweight schedule restored to the same config-driven fallback.
-- ---------------------------------------------------------------------------
