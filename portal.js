(function(){
  ...
  const THEME_KEY_V616AH="ta_employee_portal_theme_v616ah";
  ...
  let timeMonthV616AC=new Date(new Date().getFullYear(),new Date().getMonth(),1);
  let timeLoadedMonthKeyV616AC="";
  let attendanceLoadedRangeKeyV616AC="";
  ...
  const PORTAL_SYNC_INTERVAL_V61513=60000;
  ...
  function portalThemeV616AH(){
    try{return localStorage.getItem(THEME_KEY_V616AH)==="dark"?"dark":"light";}catch(_){return"light";}
  }
  function syncPortalThemeV616AH(theme=portalThemeV616AH()){
    const dark=theme==="dark";
    document.body.classList.toggle("theme-dark",dark);
    document.body.classList.remove("dark","dark-mode");
    document.documentElement.style.colorScheme=dark?"dark":"light";
    const meta=document.querySelector('meta[name="theme-color"]');
    if(meta)meta.setAttribute("content",dark?"#0F283D":"#123A5A");
    [$("portalThemeBtnV616AH"),$("portalThemeBtnAuthV616AH")].filter(Boolean).forEach(btn=>{
      btn.setAttribute("aria-pressed",String(dark));
      btn.setAttribute("aria-label",dark?"เปิด Light Mode":"เปิด Dark Mode");
      btn.title=dark?"Light Mode":"Dark Mode";
    });
  }
  function togglePortalThemeV616AH(){
    const next=portalThemeV616AH()==="dark"?"light":"dark";
    try{localStorage.setItem(THEME_KEY_V616AH,next);}catch(_){}
    syncPortalThemeV616AH(next);
  }
  ...
  function renderScheduleSummary(){
    ...
    if(!box)return;
    box.innerHTML=`
      <div class="portal-shift-legend morning"><i class="portal-calendar-legend-original-v616ac">☀</i><span>เช้า</span></div>
      <div class="portal-shift-legend night"><i class="portal-calendar-legend-original-v616ac">☾</i><span>ดึก</span></div>
      <div class="portal-shift-legend off"><i class="portal-calendar-legend-original-v616ac">☂</i><span>หยุด</span></div>
      <div class="portal-shift-legend leave"><i class="portal-calendar-legend-original-v616ac">▤</i><span>ลา</span></div>
      <div class="portal-shift-legend holiday"><i class="portal-calendar-legend-original-v616ac">⌂</i><span>นักขัตฯ</span></div>`;
  }
  ...
  function pad2V616AC(n){return String(n).padStart(2,"0");}
  function monthKeyV616AC(d=timeMonthV616AC){
    return `${d.getFullYear()}-${pad2V616AC(d.getMonth()+1)}`;
  }
  function timeMonthRangeV616AC(){
    const y=timeMonthV616AC.getFullYear(),m=timeMonthV616AC.getMonth();
    const start=`${y}-${pad2V616AC(m+1)}-01`;
    const last=new Date(y,m+1,0).getDate();
    let end=`${y}-${pad2V616AC(m+1)}-${pad2V616AC(last)}`;
    if(monthKeyV616AC()===today().slice(0,7))end=today();
    return{startDate:start,endDate:end};
  }
  function syncTimeMonthControlsV616AC(){
    const picker=$("portalTimeMonthV616AC");
    if(picker){
      picker.value=monthKeyV616AC();
      picker.max=today().slice(0,7);
    }
    const next=$("portalTimeNextMonthV616AC");
    if(next){
      next.disabled=monthKeyV616AC()>=today().slice(0,7);
      next.setAttribute("aria-disabled",String(next.disabled));
    }
  }
  function setTimeMonthV616AC(value){
    const raw=String(value||"").trim();
    if(!/^\d{4}-\d{2}$/.test(raw))return false;
    if(raw>today().slice(0,7)){
      toast("เวลาทำงานเลือกได้ถึงเดือนปัจจุบัน","info");
      return false;
    }
    const [y,m]=raw.split("-").map(Number);
    timeMonthV616AC=new Date(y,m-1,1);
    timeCalendarRowsV616AB=[];
    attendanceByDateV61503.clear();
    attendanceLoadErrorV61504="";
    timeTabLastLoadedAtV616AB=0;
    timeLoadedMonthKeyV616AC="";
    attendanceLoadedRangeKeyV616AC="";
    syncTimeMonthControlsV616AC();
    return true;
  }
  ...
  async function loadAttendanceRangeV61503({force=false,startDate=null,endDate=null}={}){
    startDate=startDate||addDays(today(),-31);
    endDate=endDate||today();
    const rangeKeyV616AC=`${startDate}|${endDate}`;

    if(!force&&attendanceByDateV61503.size&&attendanceLoadedRangeKeyV616AC===rangeKeyV616AC){
      attendanceLoadErrorV61504="";
      renderTime();
    ...
    attendanceLoadPromiseV61514=(async()=>{
      try{
        ...
        attendanceLoadErrorV61504="";
        attendanceLoadedRangeKeyV616AC=rangeKeyV616AC;
        renderTime();
        return attendanceByDateV61503;
      }catch(err){
  ...
  async function loadTimeTabV616AB({force=false}={}){
    ...
    const selectedMonthV616AC=monthKeyV616AC();
    const stale=Date.now()-timeTabLastLoadedAtV616AB>60000;
    if(
      !force
      &&!stale
      &&timeCalendarRowsV616AB.length
      &&timeLoadedMonthKeyV616AC===selectedMonthV616AC
    ){
      renderTime();
      return timeCalendarRowsV616AB;
    }

    const {startDate,endDate}=timeMonthRangeV616AC();
    const label=timeMonthV616AC.toLocaleDateString("th-TH",{month:"long",year:"numeric"});
    timeLoadStateV616AB("loading",`กำลังโหลดเวลาทำงาน • ${label}`);
    renderTime();

    timeTabLoadPromiseV616AB=(async()=>{
      let calendarError=null;
      try{
        // Month change must never reuse Attendance from another range.
        if(timeLoadedMonthKeyV616AC!==selectedMonthV616AC){
          attendanceByDateV61503.clear();
          attendanceLoadErrorV61504="";
          attendanceLoadedRangeKeyV616AC="";
        }
        ...
        const [calendarResult]=await Promise.allSettled([
          ...
          loadAttendanceRangeV61503({
            force:Boolean(force)||timeLoadedMonthKeyV616AC!==selectedMonthV616AC,
            startDate,
            endDate
          })
        ]);
        ...
        if(attendanceLoadErrorV61504){
          timeLoadStateV616AB(
            "warning",
            timeCalendarRowsV616AB.length
              ?`แสดงข้อมูล ${label} แล้ว • Attendance ละเอียดยังรออัปเดต`
              :`Attendance ${label} โหลดไม่สำเร็จ`
          );
        }else if(calendarError){
          timeLoadStateV616AB("warning",`แสดง Attendance ${label} แล้ว • ข้อมูลกะบางส่วนโหลดไม่สำเร็จ`);
        }else if(timeCalendarRowsV616AB.length){
          timeLoadStateV616AB("ready",`${label} • ${timeCalendarRowsV616AB.length} วัน`);
        }else{
          timeLoadStateV616AB("empty",`ไม่พบข้อมูลเวลาทำงานใน ${label}`);
        }
        return timeCalendarRowsV616AB;
      }catch(err){
        console.warn("FIX16AC time month loader",err);
  ...
  async function applyPortalSyncDeltaV61513(
    ...
    if(requestChanged && active==="requests"){
      jobs.push(
        loadRequests()
      );
    }

    if(notificationChanged && active==="notifications"){
      jobs.push(
        loadNotifications()
      );
    }else{
  ...
  async function refreshAll(){loading(true,"กำลังโหลดข้อมูลของคุณ...");try{attendanceByDateV61503.clear();attendanceLoadErrorV61504="";timeCalendarRowsV616AB=[];timeTabLastLoadedAtV616AB=0;timeLoadedMonthKeyV616AC="";attendanceLoadedRangeKeyV616AC="";sameShiftTeamCacheV61509.clear();certificationStateCacheV61509.clear();partialLeaveByDateV61511.clear();rawPunchCacheV61501.clear();if(!homeFocusDateV61509)homeFocusDateV61509=today();await loadCalendar();await loadSameShiftTeamV61509(homeFocusDateV61509,{force:true});portalHydratedV61514=true;await setPortalSyncBaselineV61513();startPortalSyncV61513();}catch(e){if(String(e?.message||"").includes("PORTAL_SESSION_INVALID")){localStorage.removeItem(SESSION_KEY);showAuth();toast("Session หมดอายุ กรุณาเข้าสู่ระบบใหม่","warning");}else toast(friendly(e),"error");}finally{loading(false);renderToday();renderSameShiftTeamV61509(sameShiftTeamCacheV61509.get(homeFocusDateV61509)||null,homeFocusDateV61509);}}
  ...
  function openQuickRequestChooserV616AI(){
    const modal=$("portalQuickRequestModalV616AI");
    if(!modal)return;
    modal.classList.remove("hidden");
    document.body.classList.add("portal-modal-open-v616ai");
  }
  function closeQuickRequestChooserV616AI(){
    $("portalQuickRequestModalV616AI")?.classList.add("hidden");
    if($("portalRequestModal")?.classList.contains("hidden"))document.body.classList.remove("portal-modal-open-v616ai");
  }
  ...
  async function openRequest(type="TIME_ISSUE",date=today(),existing=null){
    ...
    requestAnimationFrame(()=>{const scroller=$("portalRequestScrollV616AM");if(scroller){scroller.scrollTop=0;scroller.scrollLeft=0;}});
    document.body.classList.add("portal-modal-open-v616ai");
  ...
  function bind(){syncPortalThemeV616AH();$("portalThemeBtnV616AH")?.addEventListener("click",togglePortalThemeV616AH);$("portalThemeBtnAuthV616AH")?.addEventListener("click",togglePortalThemeV616AH);document.querySelectorAll("[data-auth-tab]").forEach(b=>b.addEventListener("click",()=>setAuthTab(b.dataset.authTab)));$("portalActivateForm").addEventListener("submit",activate);$("portalLoginForm").addEventListener("submit",login);$("portalLogoutBtn").addEventListener("click",logout);$("portalRefreshBtn").addEventListener("click",refreshAll);document.addEventListener("click",async e=>{const homeDay=e.target.closest("[data-home-day-v61509]");if(homeDay){await setHomeFocusDateV61509(Number(homeDay.dataset.homeDayV61509||0));return;}const n=e.target.closest("[data-portal-nav]");if(n){navigate(n.dataset.portalNav);return;}const q=e.target.closest("[data-request-quick]");if(q){closeQuickRequestChooserV616AI();openRequest(q.dataset.requestQuick,today());return;}const cal=e.target.closest("[data-calendar-date]");if(cal){selectedCalendarDate=cal.dataset.calendarDate;renderCalendar();await loadCalendarPunchDetailV61501(selectedCalendarDate);return;}const syncSchedule=e.target.closest("[data-schedule-sync-v61507]");if(syncSchedule){await syncScheduleV61507({force:true});return;}const retryAtt=e.target.closest("[data-retry-attendance-v61504]");if(retryAtt){attendanceByDateV61503.clear();attendanceLoadErrorV61504="";timeCalendarRowsV616AB=[];timeTabLastLoadedAtV616AB=0;timeLoadedMonthKeyV616AC="";attendanceLoadedRangeKeyV616AC="";renderTime();await loadTimeTabV616AB({force:true});return;}if(e.target.closest("[data-close-quick-request-v616ai]")){closeQuickRequestChooserV616AI();return;}if(e.target.closest("[data-close-request]")){closeRequest();return;}const ed=e.target.closest("[data-edit-request]");if(ed){const req=requests.find(x=>String(x.request_id)===String(ed.dataset.editRequest));if(req)await openRequest(req.request_type,String(req.work_date).slice(0,10),req);return;}const retryDayoff=e.target.closest("[data-dayoff-retry-v61512]");if(retryDayoff){retryDayoff.disabled=true;const ok=await loadDayoffPickerV61494(iso(dayoffPickerMonth),{quiet:true});if(ok&&!dayoffLoadStateV61512.balanceError)toast("อัปเดตข้อมูลวันหยุดล่าสุดแล้ว","success");return;}const dd=e.target.closest("[data-dayoff-date]");if(dd){selectDayoffDateV61494(dd.dataset.dayoffDate);return;}const dm=e.target.closest("[data-dayoff-month-nav]");if(dm){const delta=Number(dm.dataset.dayoffMonthNav||0);if(dayoffSource&&dayoffRequestModeV61505()==="SWAP_DAYOFF"){toast("วันที่หยุดแทนต้องอยู่ในเดือนเดียวกับวันหยุดเดิม กรุณาเลือกวันหยุดแทนหรือยกเลิกวันเดิมก่อน","info");return;}const candidate=new Date(dayoffPickerMonth.getFullYear(),dayoffPickerMonth.getMonth()+delta,1);if(iso(candidate)<dayoffCurrentMonthStartV61506()){toast("Employee Portal ไม่รองรับการขอย้อนหลัง กรุณาแจ้ง Manager","warning");return;}dayoffPickerMonth=candidate;dayoffSource="";dayoffTarget="";await loadDayoffPickerV61494(iso(dayoffPickerMonth));return;}const c=e.target.closest("[data-cancel-request]");if(c){cancelRequest(c.dataset.cancelRequest);return;}const r=e.target.closest("[data-read-notification]");if(r){await markRead(r.dataset.readNotification);const requestId=r.getAttribute("data-notification-request-v61519");if(requestId)await focusPortalRequestV61519(requestId);return;}});$("portalNewRequestBtn").addEventListener("click",openQuickRequestChooserV616AI);$("portalQuickRequestLauncherV616AI")?.addEventListener("click",openQuickRequestChooserV616AI);$("portalQuickRequestModalV616AI")?.addEventListener("click",e=>{if(e.target===$("portalQuickRequestModalV616AI"))closeQuickRequestChooserV616AI();});$("portalRequestType").addEventListener("change",async()=>{const requestScrollV616AM=$("portalRequestScrollV616AM");if(requestScrollV616AM)requestScrollV616AM.scrollTop=0;fillSubtype();if($("portalRequestType").value==="TIME_ISSUE"&&$("portalRequestDate").value){await loadCertificationStateV61509($("portalRequestDate").value,{force:true});renderEvidence();}if($("portalRequestType").value==="LEAVE_REQUEST")scheduleLeavePreviewV61520();});$("portalRequestSubtype").addEventListener("change",async()=>{updateLeavePartialFieldsV61493();if($("portalRequestType").value==="DAYOFF_SWAP"){dayoffSource="";dayoffTarget="";$("portalRequestDate").value="";$("portalDayoffTargetV61491").value="";await loadDayoffPickerV61494(iso(dayoffPickerMonth));renderDayoffPickerV61494();}if($("portalRequestType").value==="LEAVE_REQUEST")scheduleLeavePreviewV61520();renderEvidence();});$("portalRequestDate").addEventListener("change",async()=>{if($("portalRequestType").value==="LEAVE_REQUEST"){if(!$("portalLeaveEndV61491").value||$("portalRequestSubtype").value==="PARTIAL_DAY")$("portalLeaveEndV61491").value=$("portalRequestDate").value;syncLeavePolicyUIV61508();scheduleLeavePreviewV61520();}if($("portalRequestType").value==="TIME_ISSUE"&&$("portalRequestDate").value){await loadCertificationStateV61509($("portalRequestDate").value,{force:true});}renderEvidence();});$("portalLeaveTypeV61491")?.addEventListener("change",()=>{syncLeavePolicyUIV61508();scheduleLeavePreviewV61520();renderEvidence();});$("portalLeaveEndV61491")?.addEventListener("change",()=>{syncLeavePolicyUIV61508();scheduleLeavePreviewV61520();renderEvidence();});$("portalLeaveStartV61491")?.addEventListener("change",()=>{syncLeavePolicyUIV61508();scheduleLeavePreviewV61520();renderEvidence();});$("portalLeaveEndTimeV61491")?.addEventListener("change",()=>{syncLeavePolicyUIV61508();scheduleLeavePreviewV61520();renderEvidence();});$("portalDayoffTargetV61491")?.addEventListener("change",renderEvidence);$("portalRequestForm").addEventListener("submit",submitRequest);document.querySelectorAll("[data-request-filter]").forEach(b=>b.addEventListener("click",()=>{requestFilter=b.dataset.requestFilter;document.querySelectorAll("[data-request-filter]").forEach(x=>x.classList.toggle("active",x===b));renderRequests();}));$("portalPrevMonth").addEventListener("click",async()=>{selectedCalendarDate="";$("portalCalendarPunchDetail")?.classList.add("hidden");scheduleMonth=new Date(scheduleMonth.getFullYear(),scheduleMonth.getMonth()-1,1);await loadCalendar();});$("portalNextMonth").addEventListener("click",async()=>{selectedCalendarDate="";$("portalCalendarPunchDetail")?.classList.add("hidden");scheduleMonth=new Date(scheduleMonth.getFullYear(),scheduleMonth.getMonth()+1,1);await loadCalendar();});$("portalThisMonth").addEventListener("click",async()=>{selectedCalendarDate="";$("portalCalendarPunchDetail")?.classList.add("hidden");scheduleMonth=new Date();await loadCalendar();});
$("portalTimePrevMonthV616AC")?.addEventListener("click",async()=>{const d=new Date(timeMonthV616AC.getFullYear(),timeMonthV616AC.getMonth()-1,1);if(setTimeMonthV616AC(`${d.getFullYear()}-${pad2V616AC(d.getMonth()+1)}`))await loadTimeTabV616AB({force:true});});
$("portalTimeNextMonthV616AC")?.addEventListener("click",async()=>{const d=new Date(timeMonthV616AC.getFullYear(),timeMonthV616AC.getMonth()+1,1);if(setTimeMonthV616AC(`${d.getFullYear()}-${pad2V616AC(d.getMonth()+1)}`))await loadTimeTabV616AB({force:true});});
$("portalTimeThisMonthV616AC")?.addEventListener("click",async()=>{const now=new Date();if(setTimeMonthV616AC(`${now.getFullYear()}-${pad2V616AC(now.getMonth()+1)}`))await loadTimeTabV616AB({force:true});});
$("portalTimeMonthV616AC")?.addEventListener("change",async e=>{if(setTimeMonthV616AC(e.target.value))await loadTimeTabV616AB({force:true});else syncTimeMonthControlsV616AC();});
$("portalTimeRefreshV616AB")?.addEventListener("click",async()=>{attendanceByDateV61503.clear();attendanceLoadErrorV61504="";timeCalendarRowsV616AB=[];timeTabLastLoadedAtV616AB=0;timeLoadedMonthKeyV616AC="";attendanceLoadedRangeKeyV616AC="";await loadTimeTabV616AB({force:true});});}
