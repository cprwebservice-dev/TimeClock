...
-- FIX: SQL Editor maintenance bypass for shift_calendar guard
-- ============================================================================
...
-- ---------------------------------------------------------------------------
-- 2) shift_calendar Guard FIX
--    SQL Editor (postgres session + no auth.uid) ต้องทำ maintenance/migration ได้
--    การเรียกจาก Web/API ยังคงตรวจ EDIT_SCHEDULE ตามเดิม
-- ---------------------------------------------------------------------------
...
declare
  ...
  v_date date;
begin
  -- Supabase SQL Editor / database owner maintenance only.
  -- session_user remains the original login role even inside SECURITY DEFINER.
  ...
  values (
    ...
    case
      when v_template = 'SPLIT_FLEX' and v_end_mode = 'ACTUAL_OUT'
        then 'V6.11.17-FIX1 AUTO_CONFIRM SPLIT_FLEX ACTUAL_OUT'
      when v_template = 'SPLIT_FLEX'
        then 'V6.11.17-FIX1 AUTO_CONFIRM SPLIT_FLEX FIXED'
      else 'V6.11.17-FIX1 AUTO_CONFIRM NORMAL'
    end,
    auth.uid(),
    auth.uid()
  )
...
-- ---------------------------------------------------------------------------
-- 7) Migrate รายการที่เคยบันทึกไว้แต่ค้างสถานะไม่ยืนยัน
-- ---------------------------------------------------------------------------
